...
func TestCheckPassesOnTheCommittedCorpus(t *testing.T) {
	...
	if !strings.Contains(out.String(), "conformance OK: 43 parse goldens, 33 run goldens, 0 TODO") {
...
func TestRegenIsIdempotentAndWritesTheScoreboard(t *testing.T) {
	...
	if !strings.Contains(out.String(), "wrote 43 parse goldens, 33 run goldens (0 TODO)") {
	...
	if len(metadata.GoRun.Implemented) != 33 || metadata.RunCases != 33 || len(metadata.GoRun.TODO) != 0 {

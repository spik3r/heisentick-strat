...
func TestCheckPassesOnTheCommittedCorpus(t *testing.T) {
	...
	if !strings.Contains(out.String(), "conformance OK: 50 parse goldens, 37 run goldens, 0 TODO") {
...
func TestRegenIsIdempotentAndWritesTheScoreboard(t *testing.T) {
	...
	if !strings.Contains(out.String(), "wrote 50 parse goldens, 37 run goldens (0 TODO)") {
	...
	if len(metadata.GoRun.Implemented) != 37 || metadata.RunCases != 37 || len(metadata.GoRun.TODO) != 0 {

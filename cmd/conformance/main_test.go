...
func TestCheckPassesOnTheCommittedCorpus(t *testing.T) {
	...
	if !strings.Contains(out.String(), "conformance OK: 50 parse goldens, 38 run goldens, 0 TODO") {
...
func TestRegenIsIdempotentAndWritesTheScoreboard(t *testing.T) {
	...
	if !strings.Contains(out.String(), "wrote 50 parse goldens, 38 run goldens (0 TODO)") {
	...
	if len(metadata.GoRun.Implemented) != 38 || metadata.RunCases != 38 || len(metadata.GoRun.TODO) != 0 {

...
func TestCheckPassesOnTheCommittedCorpus(t *testing.T) {
	...
	if !strings.Contains(out.String(), "conformance OK: 43 parse goldens, 32 run goldens, 0 TODO") {
...
func TestRegenIsIdempotentAndWritesTheScoreboard(t *testing.T) {
	...
	if !strings.Contains(out.String(), "wrote 43 parse goldens, 32 run goldens (0 TODO)") {
	...
	if len(metadata.GoRun.Implemented) != 32 || metadata.RunCases != 32 || len(metadata.GoRun.TODO) != 0 {

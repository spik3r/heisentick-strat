...
| Named level sweep setup phrases (entry `when` rules, stop) | `strat/implementations/browser-runtime/compiler/parseSetups/namedLevelSweepPhrases.js`, wired from `compile/setupLowering.js` and `parseRiskManagement.js` | `dsl/parser_named_level_sweep.go`, wired from `parser_filters.go` (`parseWhen`) and `parser_weekend_extreme_fade.go` (`parseStopDirective`) |
